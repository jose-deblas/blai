...
def blai_backward_propagation(parameters, cache, X, Y, act_func):
    ...
    for l in reversed(range(1, L)):
        ...
        grads["dZ" + str(l)] = np.multiply(parameters["W" + str(l + 1)].T,
                                           grads["dZ" + str(l + 1)]) * blai_derivative_g(cache, l, act_func)
...
def blai_nn_model(X, Y, units_hidden_layers, act_func="relu", L_act_func="relu", learning_rate=1.2, num_iterations=1000,
                  print_cost=0, blai_seed=0):
    """
    Arguments:
    X -- dataset of shape (number of features, number of examples)
    Y -- labels of shape (number of output values, number of examples)
    ...
    num_iterations -- Number of iterations or epochs in gradient descent loop
